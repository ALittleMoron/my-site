...
| Backend, frontend SSR, DB, Valkey, MinIO, or the nginx edge becomes unavailable. | Docker health checks, restart policies, blue/green backend/frontend slots, and separate readiness checks reduce single-deploy downtime. The nginx edge uses an `always` restart policy and terminates its own PID 1 after a sustained local liveness failure, letting Docker recover it without a Docker-socket watchdog. | Full production observability and alerts are future work; failures outside nginx local liveness may not be detected quickly without external monitoring. A persistent edge fault can still produce a restart loop and requires operator diagnosis. |
| Expensive queries or content growth degrade performance. | The query-plan harness covers representative PostgreSQL plans; Lighthouse checks cover frontend lab performance and resource budgets; cache warming reduces cold-cache impact. | HTTP latency and throughput regressions are not gated because the site has no concrete load target. Query-count/N+1 detection, dashboards, and slow-query production thresholds are still tracked as future monitoring work. |
