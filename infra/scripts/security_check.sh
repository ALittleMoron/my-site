...
run_deploy_env_configuration_check() {
    ...
    require_file_contains "$ci_workflow" "_deploy.yaml" "private deploy workflow call"
    ...
    require_file_contains "$ci_workflow" "- dockerfile-lint" "deploy dependency on Dockerfile lint"
    require_file_contains "$ci_workflow" "- trivy-config-scan" "deploy dependency on Trivy config scan"
    require_file_contains "$ci_workflow" "- backend-docker-image-security" "deploy dependency on backend image security"
    require_file_contains "$ci_workflow" "- frontend-docker-image-security" "deploy dependency on frontend image security"
    require_file_contains "$ci_workflow" "- nginx-docker-image-security" "deploy dependency on nginx image security"
    require_file_contains "$ci_workflow" "- infra-security" "deploy dependency on infrastructure security"
    require_file_contains "$ci_workflow" "secrets: inherit" "deploy reusable workflow secrets pass-through"
