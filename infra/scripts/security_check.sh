...
run_healthcheck_configuration_check() {
    ...
    require_file_contains "$nginx_template" "img-src 'self' data: blob: \${MINIO_PUBLIC_URL}; font-src 'self' data:;" "main site CSP with private photo blob previews"
    ...
    require_file_contains "$nginx_template" "location = /knowledge-private" "exact private knowledge bucket denial"
    require_file_contains "$nginx_template" "location ^~ /knowledge-private/" "private knowledge bucket prefix denial"
    require_file_pattern_before "$nginx_template" "location ^~ /knowledge-private/" "proxy_pass http://minio_api;" "private knowledge bucket denial before MinIO proxy"
